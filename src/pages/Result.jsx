...
function Result()  {
  ...
  const onSubmitHandler = async (e) =>{
     ...
     if(input){
      const image = await ImageGenerator(input)
      if(image){
  ...
  return (
    ...
    <div>
      <div className='relative'>
        ...
        <span className={`absolute bottom-0 left-0 h-1 bg-blue-500 $ {loading ? 'w-full transution-all duration-[10s]': 'w-0'}`}/>
      </div>
    ...
    <div className='flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full'>
      ...
      className='bg-trasparent border border-zinc-900 text-black px-8 py-3 rounded-full cursor-pointer'>Generate another</p>
